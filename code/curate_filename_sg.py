...
def process_filename(original_path):
    ...
    match = re.search(r"(sub-[^/]+)/(anat/[^/]+)_space-[^/]+_T2w_seg\.nii\.gz$", original_path)
    if match:
        return f"{match.group(1)}/{match.group(2).replace('_space-other', '')}_T2w_label-SC_seg.nii.gz"
...
def process_csv(input_path, output_folder):
    ...
    output_path = os.path.join(output_folder, os.path.basename(input_path).replace("_space-other", ""))
